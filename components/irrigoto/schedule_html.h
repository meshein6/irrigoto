...
.sol .lbl:first-child{margin-top:0;}
...
.num u{position:absolute;right:10px;top:50%;transform:translateY(-50%);font-size:12px;
...
.enabled-row{margin-top:12px;padding-top:12px;border-top:1px solid var(--border);}
/* Per-entry path preview (b535) */
.ent-path{display:flex;gap:10px;align-items:center;margin-top:10px;
...
function normalizeEntry(e) {
  return {
    ...
    solution_every_n: Math.min(20, Math.max(2, +e.solution_every_n || 3)),
    solution_speed: [60,80,100].includes(+e.solution_speed) ? +e.solution_speed : 100,
    solution_pulse: e.solution_pulse ? 1 : 0,
    solution_pulse_on_s: Math.min(600, Math.max(1, +e.solution_pulse_on_s || 2)),
    solution_pulse_off_s: Math.min(600, Math.max(1, +e.solution_pulse_off_s || 4)),
  };
}
...
// Which bottle pumps first for this entry (lowest selected). The device
// rotates through the set across runs; the estimate uses the first.
function firstBottle(mask) { for (let b = 1; b <= 3; b++) if (mask & (1 << (b-1))) return b; return 1; }
function bottleCount(mask) { return [1,2,4].filter(b => mask & b).length; }

// mL per run = rate(bottle, speed) x pump-on seconds. Pump-on = run length
// minus the 10 s start delay, scaled by the on/off ratio when pulsing.
function estimateMl(e) {
  if (!state.rates || !e.est_min) return null;
  const rate = +state.rates[firstBottle(e.solution_bottles) - 1][SPEED_IDX[e.solution_speed]] || 0;
  if (rate <= 0) return null;
  let pumpS = Math.max(0, e.est_min * 60 - 10);
  if (e.solution_pulse) pumpS *= e.solution_pulse_on_s / (e.solution_pulse_on_s + e.solution_pulse_off_s);
  return { ml: pumpS * rate, pumpS: pumpS };
}

function fmtSecs(s) { const m = Math.floor(s/60), r = Math.round(s%60); return m ? m+' min'+(r?' '+r+' s':'') : r+' s'; }

function updateSolutionEst(card, e) {
  const l = card.querySelector('.est-l'), v = card.querySelector('.est-v');
  if (!l || !v) return;
  const est = estimateMl(e);
  if (!e.est_min) { l.textContent = 'Run length unknown'; v.textContent = '—'; v.classList.add('unset'); return; }
  let pumpS = Math.max(0, e.est_min * 60 - 10);
  if (e.solution_pulse) pumpS *= e.solution_pulse_on_s / (e.solution_pulse_on_s + e.solution_pulse_off_s);
  l.textContent = '~' + e.est_min + ' min run \u00b7 pump on ' + fmtSecs(pumpS);
  if (!est) { v.textContent = 'calibrate bottle ' + firstBottle(e.solution_bottles); v.classList.add('unset'); return; }
  v.textContent = '~' + (est.ml < 50 ? Math.round(est.ml) : Math.round(est.ml/5)*5) + ' mL';
  v.classList.remove('unset');
}

function updateSolutionHints(card, e) {
  const bits = [1,2,3].filter(b => e.solution_bottles & (1 << (b-1)));
  card.querySelector('.bottles-hint').textContent = bits.length > 1
    ? 'Alternates ' + bits.join(', ') + ', ' + bits[0] + '\u2026 across runs'
    : 'Bottle ' + bits[0] + ' every run';
  card.querySelector('.nth-hint').textContent = (e.solution_when === 1 && bits.length > 1)
    ? 'Each bottle every ' + (e.solution_every_n * bits.length) + ' runs' : '';
}

function solutionBlockHtml(e) {
  const chips = (cls, items, sel) => items.map(([v, lbl]) =>
    '<button class="day' + (sel(v) ? ' sel' : '') + '" data-' + cls + '="' + v + '">' + lbl + '</button>').join('');
  return '<div class="sol">' +
    '<div class="toggle-row" style="padding:0">' +
      '<span class="toggle-lbl">Apply solution</span>' +
      '<label class="theme-switch"><input type="checkbox" data-k="sol_en"' + (e.solution_enabled ? ' checked' : '') + '>' +
      '<span class="theme-slider"></span></label>' +
    '</div>' +
    '<div class="sol-body' + (e.solution_enabled ? '' : ' off') + '">' +
      '<span class="lbl">Bottles</span>' +
      '<div class="days" data-k="bottles">' + chips('b', [[1,'Bottle 1'],[2,'Bottle 2'],[3,'Bottle 3']], b => e.solution_bottles & (1 << (b-1))) + '</div>' +
      '<div class="hint bottles-hint"></div>' +
      '<span class="lbl">Pump speed</span>' +
      '<div class="days" data-k="speed">' + chips('v', [[60,'Low'],[80,'Medium'],[100,'Full']], v => e.solution_speed === v) + '</div>' +
      '<div class="hint">Low is the slowest the pump will turn under load.</div>' +
      '<span class="lbl">When</span>' +
      '<div class="days" data-k="when">' + chips('w', [[0,'Every run'],[1,'Every Nth run']], w => e.solution_when === w) + '</div>' +
      '<div class="sub' + (e.solution_when === 1 ? ' on' : '') + '" data-k="nth-body">' +
        '<div class="row" style="margin:0"><div class="col"><label>Every</label>' +
          '<div class="num"><input type="number" data-k="nth" min="2" max="20" step="1" inputmode="numeric" value="' + e.solution_every_n + '"><u>runs</u></div></div><div class="col"></div></div>' +
        '<div class="hint nth-hint"></div>' +
      '</div>' +
      '<div class="toggle-row" style="margin-top:12px;padding:0">' +
        '<span class="toggle-lbl">Pulse solution</span>' +
        '<label class="theme-switch"><input type="checkbox" data-k="pulse"' + (e.solution_pulse ? ' checked' : '') + '>' +
        '<span class="theme-slider"></span></label>' +
      '</div>' +
      '<div class="sub' + (e.solution_pulse ? ' on' : '') + '" data-k="pulse-body">' +
        '<div class="row" style="margin:0">' +
          '<div class="col"><label>On</label><div class="num"><input type="number" data-k="on" min="1" max="600" step="1" inputmode="numeric" value="' + e.solution_pulse_on_s + '"><u>s</u></div></div>' +
          '<div class="col"><label>Off</label><div class="num"><input type="number" data-k="off" min="1" max="600" step="1" inputmode="numeric" value="' + e.solution_pulse_off_s + '"><u>s</u></div></div>' +
        '</div>' +
        '<div class="hint">A deeper watering target runs longer, so the same on/off cycle delivers more solution. Re-check this if you change Depth.</div>' +
      '</div>' +
      '<div class="est"><span class="est-l"></span><b class="est-v"></b></div>' +
    '</div>' +
  '</div>';
}

function wireSolutionBlock(card, idx) {
  const e = state.entries[idx];
  const body = card.querySelector('.sol-body');
  const refresh = () => { updateSolutionHints(card, e); updateSolutionEst(card, e); };
  const clampNum = (el, lo, hi) => { let v = parseInt(el.value, 10); if (!(v >= lo)) v = lo; if (v > hi) v = hi; return v; };
  card.querySelector('[data-k="sol_en"]').addEventListener('change', ev => {
    e.solution_enabled = ev.target.checked ? 1 : 0;
    body.classList.toggle('off', !ev.target.checked);
  });
  card.querySelectorAll('[data-k="bottles"] .day').forEach(btn => btn.addEventListener('click', () => {
    const bit = 1 << (+btn.dataset.b - 1);
    if ((e.solution_bottles & bit) && bottleCount(e.solution_bottles) === 1) return;  // keep one
    e.solution_bottles ^= bit;
    btn.classList.toggle('sel', !!(e.solution_bottles & bit));
    refresh();
  }));
  card.querySelectorAll('[data-k="speed"] .day').forEach(btn => btn.addEventListener('click', () => {
    e.solution_speed = +btn.dataset.v;
    card.querySelectorAll('[data-k="speed"] .day').forEach(x => x.classList.toggle('sel', x === btn));
    refresh();
  }));
  card.querySelectorAll('[data-k="when"] .day').forEach(btn => btn.addEventListener('click', () => {
    e.solution_when = +btn.dataset.w;
    card.querySelectorAll('[data-k="when"] .day').forEach(x => x.classList.toggle('sel', x === btn));
    card.querySelector('[data-k="nth-body"]').classList.toggle('on', e.solution_when === 1);
    refresh();
  }));
  const nth = card.querySelector('[data-k="nth"]');
  nth.addEventListener('input', () => { e.solution_every_n = clampNum(nth, 2, 20); refresh(); });
  nth.addEventListener('blur',  () => { nth.value = e.solution_every_n; });
  card.querySelector('[data-k="pulse"]').addEventListener('change', ev => {
    e.solution_pulse = ev.target.checked ? 1 : 0;
    card.querySelector('[data-k="pulse-body"]').classList.toggle('on', !!e.solution_pulse);
    refresh();
  });
  [['on','solution_pulse_on_s'],['off','solution_pulse_off_s']].forEach(([k, f]) => {
    const el = card.querySelector('[data-k="' + k + '"]');
    el.addEventListener('input', () => { e[f] = clampNum(el, 1, 600); refresh(); });
    el.addEventListener('blur',  () => { el.value = e[f]; });
  });
  refresh();
}

// ── Path preview per entry (b535) ───────────────────────────────────────────
// /api/schedule's zone list carries only {id,name}, so geometry comes from
// one /api/all fetch, cached for the page. Thumbnails draw lazily -- there
// can be 32 entries and each one clips its rings against the polygon.
let zoneGeom = {};   // zone id (0-based) -> {points, act_max_throw, ...}
let pathObserver = null;

async function loadZoneGeom(){
  try {
    const d = await fetch('/api/all', {cache:'no-store'}).then(r => r.json());
    (d.zones || []).forEach(z => { zoneGeom[z.id] = z; });
    drawVisiblePaths();
  } catch(e){}
}

function entryPathOpts(e){
  const z = zoneGeom[e.zone - 1] || {};
...
function drawEntryPath(cv){
  ...
  if (!z || !z.points || z.points.length < 2) {
...
function drawVisiblePaths(){
  document.querySelectorAll('#entries canvas.ent-thumb').forEach(cv => {
    cv.dataset.drawn = '';
    drawEntryPath(cv);
  });
}

function observeEntryPath(cv){
  if (!('IntersectionObserver' in window)) { drawEntryPath(cv); return; }
  if (!pathObserver) {
    pathObserver = new IntersectionObserver(entries => {
      entries.forEach(en => { if (en.isIntersecting) drawEntryPath(en.target); });
    }, {rootMargin: '120px'});
  }
  pathObserver.observe(cv);
}
...
function renderEntry(e, idx) {
  ...
  card.querySelector('[data-k="zone"]').addEventListener('change', ev => {
    ...
    const c = card.querySelector('canvas.ent-thumb');   // b535
    if (c) { c.dataset.drawn = ''; drawEntryPath(c); }
  });
  ...
  card.querySelector('[data-k="depth"]').addEventListener('change', ev => {
    ...
    updateSolutionEst(card, state.entries[idx]);   // est_min refreshes on save
  });
  ...
  card.querySelectorAll('[data-k="days"] .day').forEach(btn => {
  ...
  card.querySelectorAll('.day-preset').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      card.querySelectorAll('[data-k="days"] .day').forEach(d => {
  ...
  wireSolutionBlock(card, idx);
  const _cv = card.querySelector('canvas.ent-thumb');
  if (_cv) observeEntryPath(_cv);          // b535
  return card;
}
...
function entriesToText(entries) {
  return entries.map(e =>
    [e.zone, e.mode, e.depth, e.hour, e.minute, e.days_mask, e.enabled,
     e.solution_enabled, e.solution_bottles, e.solution_when, e.solution_every_n,
     e.solution_speed, e.solution_pulse, e.solution_pulse_on_s, e.solution_pulse_off_s].join(',')
  ).join(';');
}
