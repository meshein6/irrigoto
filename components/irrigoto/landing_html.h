...
/* Apply solution block (b534) -- chips/toggles match the schedule page */
  ...
  <section>
    ...
    <div class="card">
      ...
      <div class="zone-actions">
        ...
        <a class="btn" href="/bottle_cal">&#129514; Bottle cal</a>
      </div>
    </div>
  </section>
  <section>
...
<div id="modal-bg" onclick="if(event.target===this)closeModal()">
  <div id="modal">
    ...
    <!-- b534: Apply solution (manual run). Hidden for chase/demo. -->
    <div id="sol-block">
...
let lastSolution=null;   // b534: /api/all "solution" object
...
// ── Apply solution on a manual run (b534) ────────────────────────────────────
...
let solRates=null, solEstMin=0, solEstReq=0;
function solLoadRates(){
  fetch('/api/solution_cal',{cache:'no-store'}).then(r=>r.json()).then(d=>{solRates=d.rates||null; solEst();}).catch(()=>{});
}
function solRestore(){
...
function solModeChanged(){
  const applicable = !!SOL_MODE_MAP[selModeDat];
...
async function startWater(){
  ...
  const sol=solState();
  if (sol.enabled && SOL_MODE_MAP[selModeDat]) {
    solSave();
...
async function refresh(){
  try{
    ...
    renderZones(d.zones);
    lastSolution = d.solution || null;   // b534
...
function tickCountdown(){
  ...
  // b534: show the dose alongside the countdown once it is armed.
