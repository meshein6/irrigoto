...
<main>
  ...
  <section>
    ...
    <div class="card">
      ...
      <div class="dev-row" title="Skip the start-of-run pressure check on a regulated supply">
        <span class="dev-label">Supply regulated
          <button class="info-btn" onclick="toggleSupplyHelp()" aria-expanded="false"
                  aria-controls="supply-help" aria-label="What Supply regulated does">i</button>
        </span>
        <label class="theme-switch">
      ...
      <div class="mode-help" id="supply-help" style="margin:0 0 10px">
        <b>Supply regulated.</b> Turn on if your water comes from city mains or goes
        ...
        start of each run &mdash; which can spray past the edge of small zones &mdash;
        and trusts the pressure calibration. Leave off for a well pump or pressure
        tank, where pressure rises and falls as the pump cycles; there the check
        measures today's supply so the run can adjust. If runs start landing short or
        long after turning this on, recalibrate pressure or turn it back off.
      </div>
...
const SOL_MODE_MAP={'1':[0,1],'2':[0,2],'3':[0,2],'5':[1,1],'6':[1,2],'7':[2,1],'8':[3,1]};
...
function solModeChanged(){
  ...
  const [m,d]=SOL_MODE_MAP[selModeDat];
...
// b535: regulated-supply switch. Skips the start-of-run pressure check.
...
function toggleSupplyHelp(){
  ...
  const b = document.querySelector('#supply-cb') && h.previousElementSibling
            ? h.previousElementSibling.querySelector('.info-btn') : null;
  if (b) b.setAttribute('aria-expanded', on ? 'true' : 'false');
}
// ── Run history (b535) ──────────────────────────────────────────────────────
// /api/runs returns raw CSV, newest first, no header. Columns must stay in
// step with RUNS_CSV_HEADER in irrigoto.c.
...
const DEPTH_TXT = ['','1/8\u2033','1/4\u2033','3/8\u2033','1/2\u2033','5/8\u2033','3/4\u2033','7/8\u2033','1\u2033'];
...
async function loadRuns(){
  ...
  el.innerHTML = rows.map(line => {
    ...
    const depth = DEPTH_TXT[+c[RUN_COL.depth]] || '';
    const vol = parseFloat(c[RUN_COL.vol]);
    return '<div class="run-row">' +
      '<span class="run-when">' + esc(when) + '</span>' +
      '<span class="run-what">' + esc(c[RUN_COL.zone] || '') +
        '<small>' + esc(c[RUN_COL.mode] || '') + (depth ? ' ' + depth : '') +
        ' \u00b7 ' + (+c[RUN_COL.dur]||0).toFixed(0) + ' min \u00b7 ' + esc(c[RUN_COL.trigger]||'') +
        (ok ? '' : ' \u00b7 <span style="color:var(--orange)">' + esc(st.replace(/_/g,' ')) + '</span>') +
        '</small></span>' +
...
solLoadRates();
loadRuns();   // b535: History card, once at load
async function refresh(){
  try{
    ...
    // b535: refresh History when a run ends (watering true -> false).
    if (_wasWatering && !d.watering) loadRuns();
