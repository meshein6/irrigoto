...
/* Mode + Depth pickers (b535). Replaces the fixed preset grid so any mode
   can run at any depth, the way schedule entries and HA already work. */
...
function openModal(id, name){
  ...
  // b535: reopen with the last mode/depth this browser used.
  restoreModePrefs();
...
// POST /zone/water has always accepted -- the schedule and HA already send it.
// Demo and Chase carry no depth.
...
async function startWater(){
  ...
  if (selModeDat === 'c') {
    ...
    body += '&depth=' + selDepth;   // eighths of an inch; Demo/Chase send none
  }
  try{
